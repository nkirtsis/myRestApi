...
class myapi extends API
{
    ...
    public function processRequest()
    {
        if ($this->action != '') {
            ...
            } elseif ($this->method == 'PUT' && $this->action == 'performMagicSpell') {
                $this->performMagicSpell($this->id);
            } elseif ($this->method == 'PUT' && $this->action == 'drinkHealingPotion') {
    ...
    private function hitWithAxe($id)
    {
        ...
        $this->updateCreature($id, ['health'=>$newHealthValue]);
    }
    ...
    {
        $sql    = $this->database->selectStatement('creatures', ['creature_id'=>$id]);
